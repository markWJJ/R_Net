...
运行方式：
训练数据文件：train_out_*.txt 分别有500样本/2K样本/5K样本/2w样本
验证数据文件：dev_out.txt 有2W数据
测试数据文件：test_out.txt

训练：
>> python R_Net.py --mod=train --train_dir=./data/train_out_500.txt  #默认为500的样本集
模型保存地址为 ./save_model 目前训练好的模型为500样本
...
模块分析：
./data/data_convert.py #将原始json文件转换为txt的模型输入文件
./save_model/    #模型保存路径，目前只有500样本的训练好的模型
